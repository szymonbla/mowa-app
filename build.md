...
- Decide the fate of the uncommitted `diagnose()` + `test/eval/` work: give it a
  spec and ticket, or commit as is. It stays out of `byok`.
...
- Commit or not: workflow files and `test/eval/` are still untracked on `main`.
